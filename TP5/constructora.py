...
def simular_proyecto(num_experimentos, num_corridas):
    ...
    # print("debug, en simular exp:", tiempos_corridas)
    # print("debug, en simular exp:", criticidad_accesos)
...
def calcular_ic99_exp(tiempos_experimentos):
    print("tiempos exp", tiempos_experimentos)
    tiempo_promedio = calcular_tiempo_promedio(tiempos_experimentos)
    desvio_estandar = np.std(tiempo_promedio)
    print("debug desvio", desvio_estandar)
    print("debug promedio", tiempo_promedio)
...
def calcular_criticidad_accesos(total_corridas, criticidad_accesos_experimentos):
    ...
    for criticidad_accesos in criticidad_accesos_experimentos:
        for key, value in criticidad_accesos.items():
            criticidad_accesos_totales[key] += value / total_corridas
...
def graficar_histogramas(tiempo_proyecto, tiempos_experimentos):
    ...
    plt.title('Distribucion del tiempo de realizacion del proyecto')
    ...
    plt.title('Distribucion de promedios de los 30 experimentos')
    plt.xlabel('Promedio de tiempo de finalizacion del proyecto')
    plt.ylabel('Frecuencia')
    plt.grid(True)
